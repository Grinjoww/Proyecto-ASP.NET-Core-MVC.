...
<head>
    ...
    <style>
        ...
        .ok { color: #1e7e34; background: #eafaf0; padding: 10px 14px;
...
<c:if test="${param.registrado == 'true'}">
    <div class="ok">Cuenta creada con exito. Ya puedes iniciar sesion.</div>
</c:if>

<%-- Mensaje tras cerrar sesion --%>
<c:if test="${param.logout == 'true'}">
    <div class="ok">Sesion cerrada correctamente.</div>
</c:if>

<%-- Mensaje de error de login --%>
<c:if test="${not empty errorLogin}">
    <div class="error"><c:out value="${errorLogin}"/></div>
</c:if>

<form method="POST" action="${pageContext.request.contextPath}/login">

    <%-- Token CSRF generado en el Servlet y guardado en sesion --%>
